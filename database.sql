...
CREATE OR REPLACE FUNCTION getProducts() RETURNS table(product_name VARCHAR, description varchar, price DECIMAL, stockQty INT, category_name varchar) AS $$
  SELECT products.product_name, products.description, products.price, products.stockQty, categories.category_name FROM products INNER JOIN categories ON categories.category_id = products.category_id;
$$ LANGUAGE sql;
